...
for (i in 1:num.otu){
    ...
    colnames(data.fit)[1:2] = c('abundance', 'event')
    fit = coxphf(Surv(time = abundance, event =  event) ~ Oral_SPF10, data = data.fit)
...
permutation.coxphf = function(Round = 100){
    ...
    for (i in 1:Round){
        ...
        fit.permuted = rep(NA, num.otu)
        permuted.cov = sample(MOUTH.meta$Oral_SPF10, num.subject, replace = FALSE)
...
```{r}
set.seed(1234)
onepermute = sample(MOUTH.meta$Oral_SPF10, num.subject, replace = FALSE)
```
...
R11 = Permutations[11,]
Reps = rep(abs(R11), each = 30000-1)

p.perms = colMeans(matrix(Reps, nrow = 30000-1) < as.matrix(abs(Permutations[-11,])))

for (i in 1:num.otu){
    
#    p.perm = sum(Permutations[,i] >= Permutations[11,][i])
...
#p.perms = colMeans(matrix(Reps, nrow = 30000-1) < as.matrix(abs(Permutations[-11,])))
p.perms = colMeans(matrix(rep(abs(fit.coxphf[,1]),each = 30000), nrow = 30000)<as.matrix(abs(Permutations)))
